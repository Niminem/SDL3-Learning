...
proc main =
    ...
    let
        vertShader = gpu.loadShader(currentSourcePath.parentDir() / "triangle.spv.vert",
                                                   SDL_GPU_SHADERSTAGE_VERTEX, SDL_GPU_SHADERFORMAT_SPIRV)
        fragShader = gpu.loadShader(currentSourcePath.parentDir() / "triangle.spv.frag",
...
when isMainModule:
    let
        vertPath = "glslc " & currentSourcePath.parentDir() / "triangle.glsl.vert -o " &
                           currentSourcePath.parentDir() / "triangle.spv.vert"
        compiledVert = execCmdEx(vertPath)
    if compiledVert.exitCode != 0:
        echo compiledVert.output
        quit(QuitFailure)
    let
        fragPath = "glslc " & currentSourcePath.parentDir() / "triangle.glsl.frag -o " &
                           currentSourcePath.parentDir() / "triangle.spv.frag"
