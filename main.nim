...
proc main =
    ...
    let
        vertShader = gpu.loadShader("shader.spv.vert", SDL_GPU_SHADERSTAGE_VERTEX, SDL_GPU_SHADERFORMAT_SPIRV)
        fragShader = gpu.loadShader("shader.spv.frag", SDL_GPU_SHADERSTAGE_FRAGMENT, SDL_GPU_SHADERFORMAT_SPIRV)
...
when isMainModule:
    # NOTE: must have Vulkan SDK installed via https://vulkan.lunarg.com/ (it's easy and will add the glslc compiler to your path as well)
    let compiledVert = gorgeEx("glslc shader.glsl.vert -o shader.spv.vert") # Compile the vertex shader
    if compiledVert.exitCode != 0:
        echo compiledVert.output
        quit(QuitFailure)
    let compiledFrag = gorgeEx("glslc shader.glsl.frag -o shader.spv.frag") # Compile the fragment shader
