...
proc SDL_GetWindowSize* ( window: SDL_Window, w,h: ptr int32 ): bool {.importc.}
...
const SDL_GPU_TEXTUREUSAGE_SAMPLER*               = uint32 (1'u shl 0) # Texture supports sampling.
const SDL_GPU_TEXTUREUSAGE_COLOR_TARGET*          = uint32 (1'u shl 1) # Texture is a color render target.
const SDL_GPU_TEXTUREUSAGE_DEPTH_STENCIL_TARGET*  = uint32 (1'u shl 2) # Texture is a depth stencil target.
const SDL_GPU_TEXTUREUSAGE_GRAPHICS_STORAGE_READ* = uint32 (1'u shl 3) # Texture supports storage reads in graphics stages.
const SDL_GPU_TEXTUREUSAGE_COMPUTE_STORAGE_READ*  = uint32 (1'u shl 4) # Texture supports storage reads in the compute stage.
const SDL_GPU_TEXTUREUSAGE_COMPUTE_STORAGE_WRITE* = uint32 (1'u shl 5) # Texture supports storage writes in the compute stage.
const SDL_GPU_TEXTUREUSAGE_COMPUTE_STORAGE_SIMULTANEOUS_READ_WRITE* = uint32 (1'u shl 6) # Texture supports reads and writes in the same compute shader. This is NOT equivalent to READ | WRITE.

const SDL_GPU_BUFFERUSAGE_VERTEX*                = uint32 (1'u shl 0) # Buffer is a vertex buffer.
const SDL_GPU_BUFFERUSAGE_INDEX*                 = uint32 (1'u shl 1) # Buffer is an index buffer.
const SDL_GPU_BUFFERUSAGE_INDIRECT*              = uint32 (1'u shl 2) # Buffer is an indirect buffer.
const SDL_GPU_BUFFERUSAGE_GRAPHICS_STORAGE_READ* = uint32 (1'u shl 3) # Buffer supports storage reads in graphics stages.
const SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ*  = uint32 (1'u shl 4) # Buffer supports storage reads in the compute stage.
const SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_WRITE* = uint32 (1'u shl 5) # Buffer supports storage writes in the compute stage.

const SDL_GPU_SHADERFORMAT_INVALID*: uint32 = 0.uint32
const SDL_GPU_SHADERFORMAT_PRIVATE*: uint32 = uint32 (1'u shl 0) # Shaders for NDA'd platforms.
const SDL_GPU_SHADERFORMAT_SPIRV*: uint32    = uint32(1'u shl 1) # SPIR-V shaders for Vulkan.
const SDL_GPU_SHADERFORMAT_DXBC*: uint32     = uint32 (1'u shl 2) # DXBC SM5_1 shaders for D3D12.
const SDL_GPU_SHADERFORMAT_DXIL*: uint32     = uint32 (1'u shl 3) # DXIL SM6_0 shaders for D3D12.
const SDL_GPU_SHADERFORMAT_MSL*: uint32      = uint32 (1'u shl 4) # MSL shaders for Metal.
const SDL_GPU_SHADERFORMAT_METALLIB*: uint32 = uint32 (1'u shl 5) # Precompiled metallib shaders for Metal.
...
proc SDL_GetMouseState* ( x,y: ptr cfloat ): SDL_MouseButtonFlags {.importc.}
...
type
  SDL_EventType* {.size: sizeof(uint32).} = enum # MAYBE A UINT32?
    ...
    SDL_EVENT_GAMEPAD_STEAM_HANDLE_UPDATED, ## < Gamepad Steam handle has changed
                                           ##  Touch events
    SDL_EVENT_FINGER_DOWN = 0x700, SDL_EVENT_FINGER_UP, SDL_EVENT_FINGER_MOTION, SDL_EVENT_FINGER_CANCELED, ##  0x800, 0x801, and 0x802 were the Gesture events from SDL2. Do not reuse these values! sdl2-compat needs them!
                                                                                                      ##  Clipboard events
    ...
    SDL_EVENT_PRIVATE0 = 0x4000, SDL_EVENT_PRIVATE1, SDL_EVENT_PRIVATE2, SDL_EVENT_PRIVATE3, ##  Internal events
    ...
    SDL_EVENT_USER = 0x8000, ##
                          ...
                          ##
    SDL_EVENT_LAST = 0xFFFF,    ##  This just makes sure the enum is the size of Uint32
...
proc SDL_PollEvent*(event: ptr SDL_Event): bool {.importc.}
